...
export default function Home({ parkingData }: propsType) {
  const dataRef = useRef<HTMLInputElement>(null);
  const [parkingList, setParkingList] = useState<ParkingDataType[]>(
    JSON.parse(sessionStorage.list)
  );
  ...
  const handleMapClick = (event: React.MouseEvent<HTMLElement>) => {
    ...
    if (targetElement.dataset.name !== undefined) {
      ...
      for (let i = 0; i < parkingData.length; i++) {
        if (parkingData[i].ADDR.split(" ")[0] === targetElement.id) {
          list.push(parkingData[i]);
        }
      }
      setParkingList(list);
      console.log(list);
  ...
  return (
    <div className="w-[100%] h-[100vh] flex justify-center items-center bg-gradient-to-r from-sky-200 to-blue-200">
      <div className="w-[80%] h-[90%] flex flex-col justify-center bg-white rounded-2xl drop-shadow-xl p-7">
        <div className="text-4xl h-[10%]">서울시 공영주차장 안내</div>
        <div className="flex justify-center items-center w-[100%] h-[90%]">
          <div
            className="w-[50%] h-[50%] flex justify-end items-center drop-shadow-lg"
          ...
          <div className="overflow-x-hidden w-[40%] h-[70%]" id="scroll">
            {parkingList.length === 0 ? (
              <div>결과가 없습니다</div>
            ) : (
              parkingList.map((item) => (
                <div key={item.PARKING_CODE}>
                  <Link
                    to={`${item.PARKING_CODE}`}
                    state={{
                      address: item.ADDR,
                  ...
                  >
                    <div className="w-[90%] h-[30%] m-3 p-3 bg-gray-50 hover:bg-sky-100 drop-shadow-lg rounded-2xl">
                      <div className="flex items-center">
                        <div className="text-xl mr-1">{item.PARKING_NAME}</div>
                        <div
                          className={
                            item.PAY_NM === "무료"
                              ? "bg-gray-200 text-sm px-1 rounded-xl"
                              : "bg-sky-200 text-sm px-1 rounded-xl"
                          }
                        >
                          {item.PAY_NM}
                        </div>
